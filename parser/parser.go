...
type parser struct {
	...
	annoComment *ast.CommentGroup // last annotation comment
...
func (p *parser) next() {
	...
	if p.tok == token.COMMENT {
		...
		if p.file.Line(p.pos) == p.file.Line(prev) {
			...
			// 	// thus the last comment group is an annotation comment.
			// 	p.annoComment = comment
			// }
		}
		...
		if endline+1 == p.file.Line(p.pos) {
			...
			// The next token is on the same line after comment group, thus 
			// the last comment group is the annotation comment.
			p.annoComment = comment
		}
	}
}
...
func (p *parser) parseIdent() *ast.Ident {
	...
	if p.tok == token.IDENT {
		name = p.lit
		comment = mergeComments(p.annoComment, p.leadComment)
		p.next()
	} else {
		p.expect(token.IDENT) // use expect() error handling
	}
	return &ast.Ident{NamePos: pos, Name: name, Comment: comment}
}
...
func (p *parser) getComments() *ast.CommentGroup {
	...
	if p.lineComment != nil {
		for _, c := range p.lineComment.List {
			list = append(list, c)
		}
	}
	if p.leadComment != nil {
		for _, c := range p.leadComment.List {
			list = append(list, c)
		}
	}
...
func mergeComments(comments ... *ast.CommentGroup) *ast.CommentGroup {
	var list []*ast.Comment
...
func (p *parser) printComments() {
	fmt.Println("lineComment:")
	if p.lineComment != nil {
		for _, c := range p.lineComment.List {
			fmt.Println(c.Text)
		}
	}
	fmt.Println("leadComment:")
	if p.leadComment != nil {
		for _, c := range p.leadComment.List {
			fmt.Println(c.Text)
		}
